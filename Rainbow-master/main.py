...
parser.add_argument('--target-update', type=int, default=int(800), metavar='τ', help='Number of steps after which to update target network')
...
if args.memory and os.path.exists(args.memory):
    ...
    print(f"Loaded memory from {args.memory}")
else:
    mem = ReplayMemory(args, args.memory_capacity)
...
if args.evaluate:
  ...
  for T in trange(1, args.T_max + 1):
    ...
    opponent_state = env.get_opponent_state()  # 假设环境提供这个方法
    ...
    if T >= args.learn_start:
      ...
      if T % args.evaluation_interval == 0:
        dqn.eval()  # Set DQN (online network) to evaluation mode
        avg_reward, avg_Q = test(args, env, T, dqn, val_mem, metrics, results_dir)  # Test
